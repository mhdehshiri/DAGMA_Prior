...
class DagmaLinear:
    ...
    def minimize(self, 
        ...
        for iter in range(1, max_iter+1):
            ...
            Gobj = G_score + mu * self.lambda1 * np.sign(W*B_pri) + 2 * W * M.T + mask_inc * np.sign(W)
        ...
        with tqdm(total=(T-1)*warm_iter+max_iter) as pbar:
            for i in range(int(T)):
                ...
                while success is False:
                    W_temp, success = self.minimize(self.W_est.copy(), B_pri , mu, inner_iters, s[i], lr=lr_adam, 
